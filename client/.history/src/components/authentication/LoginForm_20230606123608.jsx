      ...
      <Transition in={showForm} timeout={750} mountOnEnter unmountOnExit>
        {(state) => (
          <form
            ...
            } transition-transform duration-300 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[#1E1F1F] p-5 rounded-md shadow-sm w-[85vw] h-[45vw] flex flex-col items-center justify-center`}
          >
            ...
            />
            {errors.password && (
              <span className="mx-5 text-[red]">{errors.password.message}</span>
            )}

            <Button
              label="התחבר"
              className={"bg-[red] w-[20rem]"}
              // className={"w-[20rem] rounded-full shadow-xl"}
            />
